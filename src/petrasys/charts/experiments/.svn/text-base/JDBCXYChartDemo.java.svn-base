...
public class JDBCXYChartDemo extends ApplicationFrame {
  ...
  /**
   * Reads the data from a table called 'XYData1' in the 'JFREECHARTDB'
   * database.
   * <P>
   * You need to create this database and table before running the demo.
   * In the example
   * I have used the username 'jfreechart' and the password 'password'
   * to access the
  ...
  private XYDataset readData() {
    ...
    try {
      ...
      String sql = "SELECT `datetime`, `high`, `low` FROM `quotes1min` WHERE "
              + "`symbol`='GBP' and `datetime`>= '2010-03-16 09:00:00'";
