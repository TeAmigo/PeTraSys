...
public class TranslateDemo1 extends ApplicationFrame {
    ...
    private static class MyDemoPanel extends DemoPanel
        ...
        private JFreeChart createChart() {
            Minute nowm = new Minute();
            long nm1 = nowm.getFirstMillisecond();
            String nmlStr = DateOps.strFormat.format(new Date(nowm.getFirstMillisecond()));
            long nmw = nowm.getLastMillisecond();
            String nmwStr = DateOps.strFormat.format(new Date(nowm.getLastMillisecond()));
            XYDataset dataset1 = createDataset("Random 1", 100.0, nowm,
                    200);
            ...
            String lbStr = DateOps.strFormat.format(new Date(Double.doubleToLongBits(lb)));
            double ub = axis.getUpperBound();
            String ubStr = DateOps.strFormat.format(new Date(Double.doubleToLongBits(ub)));
            Range range = DatasetUtilities.findDomainBounds(this.dataset);
            double rl = range.getLength();
            double rlb = range.getLowerBound();
            //rpc - WORKING HERE:3/26/10 10:17 AM -
            int ic = this.dataset.getItemCount(0);
            Calendar bt = new GregorianCalendar();
            Date beginDate = axis.getMinimumDate();
            Date endDate = axis.getMaximumDate();
            bt.setTime(beginDate);
            bt.add(Calendar.MINUTE, 10);
            axis.setMaximumDate(bt.getTime());
            endDate = axis.getMaximumDate();
            axis.setRange(beginDate, bt.getTime());
            return chart1;
        }
