 ...
 * @since 1.0.13
 */
public class CrosshairOverlayADX extends AbstractOverlay implements Overlay,
...
// This is for javadoc - CrosshairOverlay co;
  ...
  private ADXChartPanel panel;
  DecimalFormat twoDForm = new DecimalFormat("#.##");

  /**
   * Default constructor.
   */
  public CrosshairOverlayADX(ADXChartPanel myPanelIn) {
    super();
    panel = myPanelIn;
   ...
   */
  public void addDomainCrosshair(CrosshairPetrasys crosshair) {
    if (crosshair == null) {
  ...
  public int rangeCrosshairExists(CrosshairPetrasys crosshair) {
    for (int i = 0; i < yCrosshairs.size(); i++) {
      Double val1 = Double.valueOf(twoDForm.format(((CrosshairPetrasys) yCrosshairs.get(i)).getValue()));
   ...
   */
  public void addRangeCrosshair(CrosshairPetrasys crosshair) {
    if (crosshair == null) {
      MsgBox.err("No Crosshair in addRangeCrosshair", "CrosshairOverlayADX");
    }
    int idx = rangeCrosshairExists(crosshair);
    if (idx  != -1) {
      removeRangeCrosshair((CrosshairPetrasys) (yCrosshairs.get(idx)));
    }
  ...
  public void removeRangeCrosshairs(String[] crosshairValues) {
    if (crosshairValues == null) {
      MsgBox.err("Null 'crosshairValue' argument.", "removeRangeCrosshair()");
    }
    boolean fireOverlay = false;
    for (int j = 0; j < crosshairValues.length; j++) {
      for (int i = 0; i < yCrosshairs.size(); i++) {
        Double val = Double.valueOf(twoDForm.format(((CrosshairPetrasys) yCrosshairs.get(i)).getValue()));
        Double valIn = Double.parseDouble(crosshairValues[j]);
        if (val.compareTo(valIn) == 0) {
          ((CrosshairPetrasys) (yCrosshairs.get(i))).removePropertyChangeListener(this);
  ...
  public Double[] getRangeCrosshairsValues() {
    ...
    for (int i = 0; i < ret.length; i++) {
      ret[i] = ((CrosshairPetrasys) yCrosshairs.get(i)).getValue();
  ...
  public String[] getDomainCrosshairsValues() {
    ...
    for (int i = 0; i < ret.length; i++) {
      long tt = (long) (((CrosshairPetrasys) xCrosshairs.get(i)).getValue());
      ret[i] = DateOps.prettyString(new Date(tt));
    }
    return ret;
  }
  ...
  private void paintEachPanel(XYPlot plot, Graphics2D g2, Rectangle2D dataArea, Rectangle2D dataAreaH) {
    ...
    while (iterator.hasNext()) {
      CrosshairPetrasys ch = (CrosshairPetrasys) iterator.next();
  ...
  public void paintOverlay(Graphics2D g2, ChartPanel chartPanel) {
    ...
    ADXChart chart = ((ADXChartPanel) chartPanel).getChart();
...
//    g2.setClip(savedClip);
  ...
  protected void drawHorizontalCrosshair(Graphics2D g2, Rectangle2D dataArea,
          double y, CrosshairPetrasys crosshair) {
...
//    g2.clip(dataArea);
  ...
  protected void drawVerticalCrosshair(Graphics2D g2, Rectangle2D dataArea,
          double x, CrosshairPetrasys crosshair) {
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof CrosshairOverlayADX)) {
      return false;
    }
    CrosshairOverlayADX that = (CrosshairOverlayADX) obj;
  ...
  public Object clone() throws CloneNotSupportedException {
    CrosshairOverlayADX clone = (CrosshairOverlayADX) super.clone();
