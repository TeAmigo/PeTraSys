...
public class MouseKeyboardController {
  ...
  public MouseKeyboardController(ADXChartPanel panel, ADXChart chart, boolean[] keys) {
    this.panel = panel;
    this.chart = chart;
    this.keys = keys;
  }
  ...
  public void keyPressed(KeyEvent e) {
    ...
    if (e.isAltDown()) {
      if (e.getKeyCode() == KeyEvent.VK_A) {
        ...
        TraderFrame tf = i.getTraderFrame();
        chart.getInstrument().getTraderFrame().setVisible(false);
        chart.getInstrument().getTraderFrame().setVisible(true);
        chart.getInstrument().getTraderFrame().setLocationToBottom();
      }
    }
  }
  ...
  public void keyReleased(KeyEvent e) {
    ...
    if (k == KeyEvent.VK_LEFT || k == KeyEvent.VK_RIGHT) {
      ...
      int minsInBar = chart.getInstrument().getPriceBars().getCompressionFactor();
  ...
  public void widenCandles() {
    CombinedDomainXYPlot combinedPlot = (CombinedDomainXYPlot) panel.adxchart.getPlot();
    XYPlot pricePlot = (XYPlot) combinedPlot.getSubplots().get(0);
  ...
  public void keyTyped(KeyEvent e) {
    if (!(e.isAltDown() || e.isControlDown())) { //Ctrl and Alt key handled elsewhere
      ...
      } else if (keys[KeyEvent.VK_L]) { //Subtract space from bottom or top of chart
        moveChartLower();
  ...
  public void chartMouseClicked(ChartMouseEvent event) {
    ...
    if(keys[KeyEvent.VK_S]) {
      panel.setupStopLossCrosshair(subChartY, "SELL-STOPLOSS");
    }
    if(keys[KeyEvent.VK_P]) {
      panel.setupProfitStopCrosshair(subChartY, "SELL-PROFITSTOP");
    }
    ...
    if (event.getTrigger().isControlDown()) {
      if (keys[KeyEvent.VK_A]) {  // Increase the relative size of subplot.
        ...
        panel.setupVerticalCrosshair(chartX);
        chart.getInstrument().getTraderFrame().instigateBuyTrade(chartX, subChartY);
        ...
        panel.setupVerticalCrosshair(chartX);
        panel.adxchart.getInstrument().getTraderFrame().instigateSellTrade(chartX, subChartY);
      }
    }
