...
import petrasys.chart.utils.CrosshairPetrasys;
import petrasys.chart.utils.CrosshairsOpsFrame;
import petrasys.chart.utils.SubPlotHorizontalCrosshairLabelGenerator;
import petrasys.chart.utils.VerticalCrosshairLabelGenerator;
import petrasys.utils.DateOps;
import petrasys.utils.TimeSeriesOps;

/**
 *
 * @author rickcharon
 */
public class ADXChartPanel extends ChartPanel implements KeyListener, ChartMouseListener {

  ADXChart adxchart;
  ADXChartFrame frame;
  CrosshairPetrasys crosshairDomain;
  CrosshairPetrasys crosshairRange;
  Crosshair crosshairRange2;
  private CrosshairOverlayADX crosshair1 = null;
  boolean[] keys = new boolean[525];
  private double rangeAdjustor = 0.025;
  CrosshairsOpsFrame crosshairsOpsFrame;
  MouseKeyboardController controller = null;
  ...
  public ADXChartPanel(JFreeChart chart, int width, int height, int minimumDrawWidth, int minimumDrawHeight, int maximumDrawWidth, int maximumDrawHeight, boolean useBuffer, boolean properties, boolean copy, boolean save, boolean print, boolean zoom, boolean tooltips) {
  ...
  public ADXChartPanel(JFreeChart chart, int width, int height, int minimumDrawWidth, int minimumDrawHeight, int maximumDrawWidth, int maximumDrawHeight, boolean useBuffer, boolean properties, boolean save, boolean print, boolean zoom, boolean tooltips) {
...
//  public ADXChartPanel(JFreeChart chart, boolean properties, boolean save, boolean print, boolean zoom, boolean tooltips) {
//    super(chart, properties, save, print, zoom, tooltips);
//  }
//
//  public ADXChartPanel(JFreeChart chart, boolean useBuffer) {
//    super(chart, useBuffer);
//  }
  @Override
  public ADXChart getChart() {
    return adxchart;
  }

  public CrosshairOverlayADX getCrosshair1() {
    return crosshair1;
  }

  public void setCrosshair1(CrosshairOverlayADX crosshair1) {
    this.crosshair1 = crosshair1;
  }

  //rpc - NOTE:7/23/10 2:14 PM - Only used by test charts,
  public ADXChartPanel(JFreeChart chart) {
    super(chart);
    setFocusable(true);
    addKeyListener(this);
    adxchart = null;
  ...
  //rpc - NOTE:7/20/10 6:50 AM - ADXChartFrame calls this ctor
  public ADXChartPanel(ADXChart chart, ADXChartFrame frameIn) {
    super(chart);
    frame = frameIn;
    adxchart = chart;
    setFocusable(true);
    ...
    crosshair1 = new CrosshairOverlayADX(this);
    addOverlay(crosshair1);
    crosshairsOpsFrame = new CrosshairsOpsFrame(this);
    setDefaultDirectoryForSaveAs(new File("/share/Images"));
    extendPopupMenu();
    controller = new MouseKeyboardController(this, adxchart, keys);
    //setupCrosshairs();
  }
  ...
  public void setRangeBoundsForPlots() {
    ...
    if (adxchart.getPlot() instanceof CombinedDomainXYPlot) {
      CombinedDomainXYPlot plot = (CombinedDomainXYPlot) adxchart.getPlot();
      ...
      double hiVal = TimeSeriesOps.maxVal(null);
  ...
  public void keyPressed1(KeyEvent e) {
    ...
    if (e.isAltDown()) {
      if (e.getKeyCode() == KeyEvent.VK_A) {
        ...
        adxchart.openControlFrame(); // Chart Menu
        ...
        adxchart.getInstrument().getTraderFrame().setVisible(false);
        adxchart.getInstrument().getTraderFrame().setLocationToBottom();
        adxchart.getInstrument().getTraderFrame().setVisible(true);

        //adxchart.getInstrument().getTraderFrame().toFront();
      }
    }
  }
  ...
  public void keyReleased1(KeyEvent e) {
    ...
    if (k == KeyEvent.VK_LEFT || k == KeyEvent.VK_RIGHT) {
      CombinedDomainXYPlot combinedPlot = (CombinedDomainXYPlot) adxchart.getPlot();
      ...
      int minsInBar = adxchart.getInstrument().getPriceBars().getCompressionFactor();
  ...
  public void setupHorizontalCrosshair(double subChartY) {
    crosshairRange = new CrosshairPetrasys(subChartY);
    SubPlotHorizontalCrosshairLabelGenerator slg = new SubPlotHorizontalCrosshairLabelGenerator(subChartY);
  ...
  public void setupStopLossCrosshair(double subChartY, String prefix) {
    ...
    for (int i = 0; i < rangeHairs.size(); i++) {
      if (((CrosshairPetrasys) rangeHairs.get(i)).isStopLossCrosshair()) {
        crosshair1.removeRangeCrosshair((CrosshairPetrasys) rangeHairs.get(i));
      }
    }
    crosshairRange = new CrosshairPetrasys(subChartY);
  ...
  public void setupProfitStopCrosshair(double subChartY, String prefix) {
    List rangeHairs = crosshair1.getRangeCrosshairs();
    for (int i = 0; i < rangeHairs.size(); i++) {
      if (((CrosshairPetrasys) rangeHairs.get(i)).isProfitStopCrosshair()) {
        crosshair1.removeRangeCrosshair((CrosshairPetrasys) rangeHairs.get(i));
      }
    }
    crosshairRange = new CrosshairPetrasys(subChartY);
  ...
  public void setupVerticalCrosshair(double chartX) {
    crosshairDomain = new CrosshairPetrasys(chartX);
    VerticalCrosshairLabelGenerator slg = new VerticalCrosshairLabelGenerator(chartX);
    Font fp = new Font("Tahoma", Font.BOLD, 14);
    crosshairDomain.setLabelFont(fp);
    crosshairDomain.setLabelBackgroundPaint(Color.white);
    crosshairDomain.setLabelAnchor(RectangleAnchor.BOTTOM_RIGHT);
    crosshairDomain.setLabelGenerator(slg);
    crosshairDomain.setLabelVisible(true);
    crosshairDomain.setPaint(Color.BLACK);
    crosshair1.addDomainCrosshair(crosshairDomain);
  ...
  public void chartMouseClickedOld(ChartMouseEvent event) {
    int mouseX = event.getTrigger().getX();
    int mouseY = event.getTrigger().getY();
    Point2D mousePoint = translateScreenToJava2D(new Point(mouseX, mouseY));
    CombinedDomainXYPlot combinedPlot = (CombinedDomainXYPlot) adxchart.getPlot();
    ...
    double chartX = domainAxis.java2DToValue(mousePoint.getX(), dataArea, //Correct x val, i.e - the datetime
            domainAxisEdge);
    double chartY = rangeAxis.java2DToValue(mousePoint.getY(), dataArea, //Correct Y val
            rangeAxisEdge);
    double subChartY = rangeAxis.java2DToValue(mousePoint.getY(), info.getSubplotInfo(subPlotIdx).getDataArea(),
            rangeAxisEdge);

    // *** B E G I N     K E Y    C O D E S ****
    int plotWeight;
    if (keys[KeyEvent.VK_E]) { // Remove all Crosshairs.
      resetOverlays();
    } else if (keys[KeyEvent.VK_W]) { // Widen the Candle body (c-key pressed), or candle outline.
    ...
    } else if (keys[KeyEvent.VK_N]) {  // Narrow the Candle body (c-key pressed), or candle outline.
      XYItemRenderer renderer = plot.getRenderer();
      if (keys[KeyEvent.VK_C]) {
        if (renderer instanceof CandlestickRenderer) {
          double width = ((CandlestickRenderer) renderer).getCandleWidth();
    ...
    } else if (keys[KeyEvent.VK_A]) { //Add space to top (t key), or bottom (b key) or both.
      if (keys[KeyEvent.VK_T]) {
        Range rg = rangeAxis.getRange();
        double length = rg.getLength();
        Range adjusted = new Range(rg.getLowerBound(), rg.getUpperBound() + (rangeAdjustor * length));
        rangeAxis.setRange(adjusted);
      } else if (keys[KeyEvent.VK_B]) {
        Range rg = rangeAxis.getRange();
        double length = rg.getLength();
        Range adjusted = new Range(rg.getLowerBound() - (rangeAdjustor * length), rg.getUpperBound());
        rangeAxis.setRange(adjusted);
      } else {
        rangeAxis.resizeRange(1 + (2 * rangeAdjustor));
      }
    } else if (keys[KeyEvent.VK_S]) { //Subtract space from top (t key), or bottom (b key) or both.
      if (keys[KeyEvent.VK_T]) {
        Range rg = rangeAxis.getRange();
        double length = rg.getLength();
        Range adjusted = new Range(rg.getLowerBound(), rg.getUpperBound() - (rangeAdjustor * length));
    ...
    } else if (keys[KeyEvent.VK_H]) { // Move chart higher (vertically).
      Range rg = rangeAxis.getRange();
      double length = rg.getLength();
      Range adjusted = new Range(rg.getLowerBound() + (rangeAdjustor * length),
              rg.getUpperBound() + (rangeAdjustor * length));
      rangeAxis.setRange(adjusted);
    } else if (keys[KeyEvent.VK_L]) { // Move chart lower (vertically).
      Range rg = rangeAxis.getRange();
      double length = rg.getLength();
      Range adjusted = new Range(rg.getLowerBound() - (rangeAdjustor * length),
              rg.getUpperBound() - (rangeAdjustor * length));
      rangeAxis.setRange(adjusted);
    } else if (keys[KeyEvent.VK_P]) {     // Invoke the Portfolio window
    }
    ...
    if (event.getTrigger().isControlDown()) {
      if (keys[KeyEvent.VK_A]) {  // Increase the relative size of subplot.
      ...
      } else if (keys[KeyEvent.VK_E]) { // Erase all Crosshairs
        //erase crosshairs
        ...
        setupVerticalCrosshair(chartX);
      } else if (keys[KeyEvent.VK_C]) {
        setupCrosshairs(chartX, chartY, subChartY, plot);
      } else if (keys[KeyEvent.VK_B]) {
        setupHorizontalPrefixedCrosshair(subChartY, "BUY");
        setupVerticalCrosshair(chartX);
        adxchart.getInstrument().getTraderFrame().instigateBuyTrade(chartX, subChartY);
      } else if (keys[KeyEvent.VK_S]) { //rpc - WORKING HERE:8/6/10 2:50 PM - SELL Trade.
        setupHorizontalPrefixedCrosshair(subChartY, "SELL");
        setupVerticalCrosshair(chartX);
        adxchart.getInstrument().getTraderFrame().instigateSellTrade(chartX, subChartY);
      }
    } else if (keys[KeyEvent.VK_Z]) { //Z just because everything else is used, and it's on left little finger
...
//      frame.setTitle(adxchart.getInstrument().getChartNameSansDates() + DateOps.prettyString(dl)
//              + " - " + DateOps.prettyString(du));
      frame.setTitle(adxchart.getInstrument().getChartNameSansDates() + "-" + DateOps.fileFormatString(dl)
              + "-to-" + DateOps.fileFormatString(du));
    }
  }
  ...
  public void doSaveAs() throws IOException {
    ...
    if (option == JFileChooser.APPROVE_OPTION) {
      ...
      ChartUtilities.saveChartAsPNG(new File(filename), this.adxchart,
              getWidth(), getHeight());
    }
  }
}
