 ...
 * @author rickcharon
 */
public class DBopsPostgresql {

  static private Connection tradesConnection = null;

  public DBopsPostgresql() {
  }
  ...
  public static Connection setuptradesConnection() {
    ...
    try {
      ...
      tradesConnection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/Trading", "rickcharon", "1plus7is8");
    } catch (Exception ex) {
      MsgBox.err2(ex);
  ...
  public static CallableStatement distinctSymsProc() {
    ...
    try {
      ret = DBopsPostgresql.setuptradesConnection().prepareCall("select * from distinctQuoteSymbols();",
  ...
  public static CallableStatement datedRangeBySymbol(String sym, Timestamp beginDate, Timestamp endDate) {
    ...
    try {
      String callStr = "select * from datedRangeBySymbol('" + sym + "', '" + beginDate + "', '"
              + endDate + "');";
      ret = DBopsPostgresql.setuptradesConnection().
  ...
  public static PreparedStatement datedRangeBySymbolAndExpiry(String sym, int expiry,
    ...
    try {
      pstmt = DBopsPostgresql.setuptradesConnection().prepareStatement("SELECT datetime, open, high,low, close, "
  ...
  public static PreparedStatement minMaxDateBySymbolAndExpiry(String symbol, int expiry) {
    PreparedStatement pstmt = null;
    try {
      pstmt = DBopsPostgresql.setuptradesConnection().prepareStatement("SELECT min(datetime) as minDate, max(datetime) as maxDate FROM quotes1min "
  ...
  public static int maxExpiryWithDataBySymbol(String symbol) {
    ...
    int expiry = 0;
    try {
      pstmt = DBopsPostgresql.setuptradesConnection().prepareStatement("SELECT max(expiry) FROM quotes1min  WHERE symbol= ?");
      pstmt.setString(1, symbol);
  ...
  public static PreparedStatement minMaxDatesBySym(String sym) {
    PreparedStatement pstmt = null;
    try {
      pstmt = DBopsPostgresql.setuptradesConnection().prepareStatement(
  ...
  public static PreparedStatement distinctSymbolInfos() {
    PreparedStatement pstmt = null;
    try {
      pstmt = DBopsPostgresql.setuptradesConnection().prepareStatement("SELECT distinct  symbol, exchange, multiplier, "
  ...
  public static PreparedStatement MultiplierAndMagnifier(String sym) {
    PreparedStatement pstmt = null;
    try {
      pstmt = DBopsPostgresql.setuptradesConnection().prepareStatement("SELECT distinct multiplier, priceMagnifier "
  ...
  public static PreparedStatement exchangeBySymbolandExpiry(String symbol, int expiry) {
    PreparedStatement pstmt = null;
    try {
      pstmt = DBopsPostgresql.setuptradesConnection().prepareStatement("SELECT exchange FROM futuresContractDetails WHERE "
              + "symbol= ? and expiry= ?");
      pstmt.setString(1, symbol);
      pstmt.setInt(2, expiry);
    } catch (SQLException ex) {
  ...
  public static List<PriceBar> getPriceDatas(String sym, java.util.Date beginDT, java.util.Date endDT,
    ...
    try {
      PreparedStatement datedRangeBySymbol =
              DBopsPostgresql.datedRangeBySymbol(sym, new Timestamp(beginDT.getTime()),
  ...
  public static String createCompressionTable(int compressionFactor) {
    ...
    try {
      PreparedStatement createCompressionTable =
              DBopsPostgresql.setuptradesConnection().prepareStatement("CREATE TABLE IF NOT EXISTS "
              + dbTableName
  ...
  public static PreparedStatement insertIntoCompressionTable(String table) {
    ...
    try {
      pstmt =
              DBopsPostgresql.setuptradesConnection().
  ...
  public static void OLDinsertIntoPaperOrdersTable(Order order) {
    ...
    try {
      pstmt =
              DBopsPostgresql.setuptradesConnection(). ////rpc - NOTE:8/15/10 11:35 AM - 22 PARAMS
  ...
  public static void main(String[] args) {
    try {
      Connection con = DBopsPostgresql.setuptradesConnection();
      con.setAutoCommit(false);
      String ul = "AUD";
      PreparedStatement pstmt = DBopsPostgresql.getExpirysForUpdate(con, ul, 20090300, 20100700);
      PreparedStatement upDateStmt = DBopsPostgresql.updateBeginEndDatesForExpiry(con);
      ResultSet res = pstmt.executeQuery();
      res.next(); //To get a lastexpiry for loop, so should be one extra early expiry
