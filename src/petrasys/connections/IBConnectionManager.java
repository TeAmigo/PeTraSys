...
public class IBConnectionManager {
   ...
   */
  public static  MySocket connect(IBWrapperAdapter caller) {

    MySocket socket = new MySocket(caller, IBConnectionManager.getClientId());

    if (!socket.isConnected()) {
      socket.eConnect(IBConnectionManager.getHost(), IBConnectionManager.getPort(), socket.getClientId());
    }
    if (!socket.isConnected()) {
      PeTraSys.writeToReport("Could not connect to TWS.");
    } else {
      PeTraSys.writeToReport("Connected to TWS.");
    }
  ...
  public static void disConnect(MySocket socket) {
    socket.disConnect();
  }
  ...
  public static void main(String args[]) {
    ...
    for (int test = 0; test < 30; test++) {
      tt = IBConnectionManager.getClientId();
      hold = tt;
    }
    for (int test = 7; test < 30; test += 7) {
      IBConnectionManager.removeClientId(test);
    }
